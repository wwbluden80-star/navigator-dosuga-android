...
class AppStateStore(private val prefs:DataStore<Preferences>){
    private val mode=stringPreferencesKey("ui_mode");private val lat=doublePreferencesKey("map_lat");private val lon=doublePreferencesKey("map_lon");private val zoom=doublePreferencesKey("map_zoom");private val bearing=doublePreferencesKey("map_bearing");private val tilt=doublePreferencesKey("map_tilt");private val liveGlass=booleanPreferencesKey("live_glass");private val theme=stringPreferencesKey("ui_theme")
    val state:Flow<PersistedUiState> = prefs.data.map{p->PersistedUiState(ActivityMode.fromWire(p[mode]),MapCameraState(p[lat]?:55.751244,p[lon]?:37.618423,p[zoom]?:9.5,p[bearing]?:0.0,p[tilt]?:0.0),p[liveGlass]?:false,p[theme]?:"system")}
